...
// Time complexity O(n*2^n) estimated
class TreeNode {
...
function findUnique_trees_recursive(start, end) {
  ...
  // consider n = 1, in this case we will have start = end = 1, this means we should have // we will have two recursive calls, findUniqueTreesRecursive(1, 0) & (2, 1)
  ...
  for (let i = start; i < end + 1; i++) {
    ...
    for (let p = 0; p < leftSubtrees.length; p++) {
      for (let q = 0; q < rightSubtrees.length; q++) {
        const root = new TreeNode(i, leftSubtrees[p], rightSubtrees[q]); result.push(root);
      }
    }
  }
  return result;
}
console.log(`Total trees: ${find_unique_trees(2).length}`); console.log(`Total trees: ${find_unique_trees(3).length}`);